$(document).ready(function () {
    ...
    $("#drafteditor-buttons").on("click", "#drafteditor-buttons-addtojournal", function () {
        ...
        if (navigator.onLine) {
            if (getCookie("currentDraft")) {
                ...
                alert('Cannot load draft. Please try saving your draft and restarting this app.')
            }
        }
        else {
            alert("Oops, it looks like you're offline. Please try again when you have an internet connection.")
        }
    });

    // Save current draft
    $("#drafteditor-buttons").on("click", "#drafteditor-buttons-save", function () {
        dt = Date.now();
        var txt = $.trim($("#drafteditor-text").val());
        ...
        if (txt !== "") {
            ...
            var draftname = getCookie("currentDraft") ? getCookie("currentDraft") : String(dt);
            ...
            db.drafts
                ...
                .then(function (draft) {
                    ...
                    // Refresh page
                    window.location.assign(window.location.pathname + "#mydrafts-title");
                });
        } else {
    ...
    $("#drafteditor-buttons").on("click", "#drafteditor-buttons-delete", function () {
        if (getCookie("currentDraft")) {
            db.drafts.delete(getCookie("currentDraft")).then(function (currentDraft) {
                // Delete currentDraft cookie
                deleteCookie('currentDraft');
                // Refresh page
            ...
            db.drafts
                .each(function (draft) {
                    items +=
                        ...
                        <a href="#drafteditor-title" class="drafts-list-item-link" id="` + draft.name + `">`
                        + tidyDate(draft.name) + `</a><p>`
                        + truncate(draft.text, 30) + `</p></div>`;
                })
                .then(function () {
    ...
    function setDraftEditorState() {
        ...
        if (getCookie("currentDraft")) var label = "Edit draft:- " + tidyDate(getCookie("currentDraft"));
